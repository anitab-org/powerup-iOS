        ...
        if avatar.getAccessoryByType(itemChosen.type) != nil && itemChosen.id == avatar.getAccessoryByType(itemChosen.type)!.id {
            ...
            if !itemChosen.purchased {
                ...
                if haveEnoughPointsToBuy(accessoryPrice: itemChosen.points) {
                    ...
                    let cannotRevertAlert = UIAlertController(title: "Warning", message: "Are you sure you want to purchase these items? You will be spending \n\(itemChosen.points) Karma Points! \n and the purchase can't be reverted.", preferredStyle: .alert)
